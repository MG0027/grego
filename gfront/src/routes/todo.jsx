  ...
  return (
    <div className={styles.todo}>
      ...
      <main className={styles.mainContent}>
        <h1>
          <img src='/images/scrum-process-icon-task-icon-time-icon-computer-icon-programming-data-floppy-disk-software-directory-line-art-png-clipart-removebg-preview.png' alt='Task Manager Icon' />
          Task Manager
        </h1>
          ...
          <div className={styles.tasklist}>
            ...
            {inCompletedTask.length > 0 ? (
              inCompletedTask.map((task, index) => {
                console.log(task);
                const { formattedDate, formattedTime } = formatDateTime(task.duedatetime);
                return (
                  <div key={index} className={styles.taskItem}>
                    ...
                    <button onClick={()=>changeStatus(task._id)} className={styles.checkb}> <span className={styles.check}><i className="bi bi-check2" style={{backgroundColor:'whitesmoke', color:'green', border:'none',}}></i></span></button>
                    <button onClick={() => removeTask(task._id)} className={styles.deleteButton}>
              ...
              <p style={{backgroundColor:'transparent', textAlign:'center'}}>No tasks available. Add a new task.</p>
            )}
          </div>
        </div>
      </main>
